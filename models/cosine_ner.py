...
class CosineSoftmaxForNer(BertPreTrainedModel):
    def __init__(self, config):
        super(CosineSoftmaxForNer, self).__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)
        self.encoder = TransformerEncoder(attn_type=config.attn_type, scaled=config.scaled, num_layers=1, dim_model=config.hidden_size, num_heads=8, dim_feedforward=config.intermediate_size, dropout=config.hidden_dropout_prob)
    ...
    def forward(
        ...
        encoder_output = self.encoder(sequence_output)
